...
import { AsyncStorage, SafeAreaView, Dimensions, ScrollView, View, Text, TextInput, Image, TouchableOpacity, StyleSheet } from 'react-native'
...
export default function addproduct(props) {
	const { menuid } = props.route.params
	...
	const addTheNewProduct = async() => {
		...
		if (info && image.uri && price) {
			options.forEach(function (option) {
				delete option['key']
			})

			const data = { userid, locationid, menuid: menuid, info: info, image, options, price }
		...
		<SafeAreaView style={{ flex: 1 }}>
			<ScrollView>
				<View style={style.box}>
					...
					<TextInput style={style.addInput} placeholder="Product name" placeholderTextColor="rgba(127, 127, 127, 0.5)" onChangeText={(info) => setInfo(info)}/>
...
const style = StyleSheet.create({
	...
	addInput: { borderRadius: 5, borderStyle: 'solid', borderWidth: 3, fontSize: 13, marginBottom: 50, padding: 5, width: '80%' },
	...
	priceBox: { flexDirection: 'row', marginBottom: 50 },
